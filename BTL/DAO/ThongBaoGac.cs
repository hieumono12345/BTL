...
namespace BTL.DAO
{
    public class ThongBaoGac
    {
        ...
        {
            DataTable data = DataProvider.Instance.ExecuteQuery("SELECT ROW_NUMBER() OVER (ORDER BY MaGac) AS 'STT', MaGac , Ngay , Hoi , Dap , NhacNho , STTDS , MaDV from NoiDungGac  where NoiDungGac.MaDV=" + MaDV);
            return data;
        }
    }
}
